...
export async function GET(
  ...
  try {
    ...
    const url = `${backendUrl.replace(/\/$/, "")}/api/services/${id}`;
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao obter serviço" },
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao atualizar serviço" },
      { status: res.status || 400 }
    );
  } catch (error) {
    console.error("Erro ao conectar com backend:", error);
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao deletar serviço" },
      { status: res.status || 400 }
    );
  } catch (error) {
    console.error("Erro ao conectar com backend:", error);
