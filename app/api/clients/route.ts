...
export async function GET(request: NextRequest) {
  try {
    ...
    // Endpoint correto do backend para listar clientes
    const url = `${backendUrl.replace(/\/$/, "")}/api/clients`;
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao obter clientes" },
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao criar cliente" },
      { status: res.status || 400 }
    );
  } catch (error) {
    console.error("Erro ao conectar com backend:", error);
