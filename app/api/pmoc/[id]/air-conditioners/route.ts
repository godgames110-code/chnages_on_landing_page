...
export async function GET(
  ...
  try {
    ...
    const url = `${backendUrl.replace(/\/$/, "")}/api/pmocs/${id}/air-conditioners`;
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao obter ar-condicionados do PMOC" },
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { error: data.errors || data.message || "Erro ao adicionar ar-condicionado ao PMOC" },
      { status: res.status || 400 }
    );
  } catch (error) {
    console.error("Erro ao conectar com backend:", error);
